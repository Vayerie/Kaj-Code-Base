...
module.exports = {
	...
	run: async (client, interaction) => {
		try {
			...
			const closingEmbed = new EmbedBuilder().setColor('Red').setTitle('Closing Ticket').setDescription('Closing ticket...');

			await channel.send({ embeds: [closingEmbed] });

			await interaction.deferReply();

			const closedEmbed = new EmbedBuilder().setColor('Red').setTitle('Ticket Closed').setDescription('This ticket has been closed.');
