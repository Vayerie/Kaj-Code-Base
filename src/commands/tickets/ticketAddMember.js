...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addUserOption((option) => option.setName('member').setDescription('The member to add to the ticket.').setRequired(true)),
	...
	run: async (client, interaction) => {
		try {
			...
			const threadMember = await channel.members.fetch(memberToAdd.id).catch((err) => {
				console.log(err);
			});
